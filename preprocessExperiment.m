...
try
    load(fullfile(expRootLocal,[expID,'_stim.mat']));
catch
    error('Stimulus parameter file not found - this experiment was probably from pre-Dec 2021.');
end
...
for iStimType = 1:length(expDat.stims)
    % within each stim are features, i.e. gratings or movies
    for iFeature = 1:length(expDat.stims(iStimType).features)
        allStimTypes_mat{end+1,1} = iStimType;
        allStimTypes_mat{end,2} = iFeature;
        % cycle through parameters of stim feature
        if strcmp (expDat.stims(iStimType).features(iFeature).name{1},'grating') 
            param_list = paramNames_gratings(4:end);
            allStimTypes_mat{end,3} = '0';
        elseif strcmp (expDat.stims(iStimType).features(iFeature).name{1},'movie') 
            param_list = paramNames_video(4:end);
            allStimTypes_mat{end,3} = '1';
        end
        for iParam = 1:length(param_list)
            if strcmp(param_list{iParam},'size')
                param_number = find(ismember(expDat.stims(iStimType).features(iFeature).params,'width'));
            else
                param_number = find(ismember(expDat.stims(iStimType).features(iFeature).params,param_list{iParam}));
            end
            allStimTypes_mat{end,iParam+3} = expDat.stims(iStimType).features(iFeature).vals{param_number};
        end
    end
end
...
% from 6.2.23 this is now derived from matlab log of stimulus info rather
% than bonvisions
writecell(allStimTypes_mat,fullfile(bvDataRoot,'StimProperties.csv'));
